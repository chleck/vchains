var vchains = new function() {
  ...
  var createCleanMethod = function(f){
    return function(){
  ...
  var createValidationMethod = function(f){
    return function(){
      ...
      if(this.errorCallback && this._error) this.errorCallback(this._error);
      return this;
    }
  }
  ...
  V.cleaner = function(){
    ...
    if(typeof arguments[0] === 'string'){
      var f = arguments[1];
      this.prototype[arguments[0]] = createCleanMethod(f);
      return;
    }
    // If called with object argument
    for(var name in arguments[0]){
      var f = arguments[0][name];
      this.prototype[name] = createCleanMethod(f);
    }
  }
  ...
  V.prototype.msg = function(){
    return this._error || this.success || '';
  }
  ...
  V.prototype.raw = function(){
    return { raw: this._raw, val: this._value, msg: this._message, err: this._error };
  }
  ...
  // Not match regexp
  V.validator('isNot', function(regexp, msg){
    if(this._value.match(regexp))
    return msg || 'Bad value';
  });
  // Email
  ...
  V.validator('isFloat', function(msg){
    if(v === '' || !this._value.match(/^(?:-?(?:[0-9]+))?(?:\.[0-9]*)?(?:[eE][\+\-]?(?:[0-9]+))?$/))
    return msg || 'Bad float value';
  });
  // Less
  V.validator('isL', function(n, msg){
    f = parseFloat(this._value);
    if(isNaN(f) || f < n)
  ...
  // Greater
  V.validator('isG', function(n, msg){
    f = parseFloat(this._value);
    if(isNaN(f) || f > n)
  ...
  // Equal
  V.validator('isEqual', function(value, msg){
  ...
  // Not in list
  V.validator('isNotIn', function(options, msg){
    if(options.indexOf(this._value) !== -1)
    return msg || 'Bad value';
  });
  ...
  */
  ...
  this.validate = function(value, msg){
